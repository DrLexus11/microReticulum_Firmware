...
| `send_to_columba.py <pn_hash> <dest_hash> [body]` | Push one message for a real client (a phone) into a node's store and stop, so the client can sync it later. Its persistent Deck sender identity and router state live under ignored `state/lxmf-columba-sender/`, never in `tools/`. |
